@suite @ZAP @chocs
Feature: Wrapper integrating with chocs-frontend
  ...
  Scenario: User logins to Chocs Service and should see default GOV.UK header along with service name so user is reassured they have signed into Government service.
    ...
    Then User should see Chocs title page Header contain logo text as GOV.UK in govuk-header__logotype-text
    Then User should see Chocs title page Header contain service name as Personal tax account in govuk-header__content


  Scenario: User logins to Chocs web application and BTA menu only shown based on the SA enrolment rule.
    Given User login to the Chocs GG Login Page
  ...
  Scenario: The Phase banner if it is shown should only be positioned at the bottom of the page directly above the footer and feedback link should be redirect to 'Send your feedback' Page
    ...
    Then Verify redirected URL should contains service name as update-your-details
...
Scenario: User should be able to see the content in Welsh language
   Given User login to the Chocs GG Login Page
  When the user clicks on 'Cymraeg' welesh language link of hmrc-account/update-your-details
  Then the user sees services relevant content of chocs in welsh language
  When the user clicks on 'English' language link of hmrc-account/update-your-details
